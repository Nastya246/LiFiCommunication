...
var ListArraySend = arrayListOf<BitSet>() //здесь храним посылки
    ...
    class MainActivitySend : AppCompatActivity() {
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            var Passw=ArrayDataNamePasw[0].toByteArray() // ключ безопасности
            var nameDevice=ArrayDataNamePasw[1].toByteArray() //имя устройства

            var keyForPassw = "LightNamePassw".toByteArray() //это ключ для шифрования ключа пароля безопасности
            var encoderKeyPassw: RC4=RC4(keyForPassw)
            var encoderResultPassw=encoderKeyPassw.Encode(Passw, Passw.size) //шифрование пароля безопасности

            var keyForNameDevice = "LightNameDevice".toByteArray() //это ключ для шифрования ключа имени устройства
            var encoderKeyDevice: RC4=RC4(keyForNameDevice)
            var encoderResultNameDevice=encoderKeyDevice.Encode(nameDevice, nameDevice.size) //шифрование имени устройства
        ...
        fun SendData(view: View?) {
            ...
            if (flagSend) {
                for (n in ListArraySend) {
                     if (flagSend)
                    SendInformation(n.toByteArray()) // ОРГАНИЗОВАТЬ ОБРАЩЕНИЕ ПОТОКА К ПЕРЕМЕННОЙ
                     /*    else {
            ...
            val taskSend1 = GlobalScope.launch() {
                while (true) {
                    if (!flagSend) {
                           /*     val showWarning = Toast.makeText(
                                    applicationContext,
        ...
        suspend fun SendInformation(dataUsersFoJack: ByteArray) {
            var buffersize = AudioTrack.getMinBufferSize(8000,  //устанавливаем частоту с запасом
                AudioFormat.CHANNEL_OUT_FRONT_LEFT, // данные идут в левый канал, поэтому моно
